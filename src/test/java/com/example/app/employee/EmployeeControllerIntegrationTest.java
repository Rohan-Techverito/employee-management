...
class EmployeeControllerIntegrationTest {
    ...
    private String departmentFixtureId;
    ...
    void setUp() throws Exception {
        jdbcTemplate.execute("TRUNCATE TABLE employees CASCADE");
        jdbcTemplate.execute("TRUNCATE TABLE departments CASCADE");
        {
            String _loc = mockMvc.perform(post("/api/v1/departments")
                            .with(user("admin").roles("ADMIN"))
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("{\"name\":\"parent-security\"}"))
                    .andExpect(status().isCreated())
                    .andReturn().getResponse().getHeader("Location");
            departmentFixtureId = _loc.substring(_loc.lastIndexOf('/') + 1);
        }
    }
    ...
    void should_return_404_when_entity_is_not_found_by_id() throws Exception {
        mockMvc.perform(get("/api/v1/employees/{id}", "00000000-0000-0000-0000-000000000000"))
    ...
    void should_return_400_when_id_format_is_invalid() throws Exception {
        mockMvc.perform(get("/api/v1/employees/{id}", "not-a-valid-uuid"))
    ...
    void should_return_404_when_updating_non_existent_entity() throws Exception {
        mockMvc.perform(put("/api/v1/employees/{id}", "00000000-0000-0000-0000-000000000000")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(payload()))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404));
    ...
    void should_return_404_when_deleting_non_existent_entity() throws Exception {
        mockMvc.perform(delete("/api/v1/employees/{id}", "00000000-0000-0000-0000-000000000000"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404));
    }
