...
class EmployeeSkillControllerIntegrationTest {
    ...
    @Autowired EmployeeSkillRepository repository;

    /** Returns the minimal valid JSON payload, substituting live parent IDs for FK fields. */
    private String payload() {
        return "{\"employeeId\":\"00000000-0000-0000-0000-000000000001\",\"skillName\":\"test-security\",\"proficiencyLevel\":\"beginner\"}";
    }
    ...
    void should_return_200_with_empty_list_when_no_entities_exist() throws Exception {
        mockMvc.perform(get("/api/v1/employee-skills"))
    ...
    void should_return_200_with_entities_after_create() throws Exception {
        mockMvc.perform(post("/api/v1/employee-skills")
    ...
    // ── GET /api/v1/employee-skills/{id} ───────────────────────────────────────────────────

    @Test
    @WithMockUser(roles = {"ADMIN"})
    void should_return_200_when_entity_exists_by_id() throws Exception {
        String location = mockMvc.perform(post("/api/v1/employee-skills")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(payload()))
                .andExpect(status().isCreated())
        ...
        mockMvc.perform(get(location))
                ...
                .andExpect(jsonPath("$.skillName").value("test-security"))
    ...
    void should_return_404_when_entity_is_not_found_by_id() throws Exception {
        mockMvc.perform(get("/api/v1/employee-skills/{id}", "00000000-0000-0000-0000-000000000000"))
    ...
    void should_return_400_when_id_format_is_invalid() throws Exception {
        mockMvc.perform(get("/api/v1/employee-skills/{id}", "not-a-valid-uuid"))
    ...
    void should_return_400_when_create_request_has_blank_string_fields() throws Exception {
        mockMvc.perform(post("/api/v1/employee-skills")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"employeeId\":\"00000000-0000-0000-0000-000000000000\",\"skillName\":\"\",\"proficiencyLevel\":\"\"}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.status").value(400));
    }

    // ── PUT /api/v1/employee-skills/{id} ────────────────────────────────────────────────────
    ...
    void should_return_404_when_updating_non_existent_entity() throws Exception {
        mockMvc.perform(put("/api/v1/employee-skills/{id}", "00000000-0000-0000-0000-000000000000")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(payload()))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404));
    ...
    // ── DELETE /api/v1/employee-skills/{id} ─────────────────────────────────────────────────
    ...
    void should_return_404_when_deleting_non_existent_entity() throws Exception {
        mockMvc.perform(delete("/api/v1/employee-skills/{id}", "00000000-0000-0000-0000-000000000000"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.status").value(404));
    }
